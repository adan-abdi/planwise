...
const StepCircle: React.FC<StepCircleProps & { state: "completed" | "active" | "pending" }> = ({ state, darkMode }) => {
  if (state === "completed") {
    return (
      <span className={`w-10 h-9 flex items-center justify-center rounded-full border-2 border-green-500 bg-green-50 dark:bg-green-900/20`}>
  ...
  if (state === "active") {
    return (
      <span className={`w-10 h-9 flex items-center justify-center rounded-full border-2 ${darkMode ? "border-blue-400" : "border-blue-600"} bg-white shadow`}>
        <span className={`block rounded-full ${darkMode ? "bg-blue-400" : "bg-blue-600"}`} style={{ width: '12px', height: '12px' }} />
      </span>
    );
  }
  return (
    <span className={`w-10 h-9 flex items-center justify-center rounded-full border-2 border-zinc-200 bg-white dark:bg-zinc-900`} style={{ backgroundColor: darkMode ? '#18181b' : '#ffffff' }} />
  );
};

const Stepper: React.FC<StepperProps> = ({ steps, current, darkMode, onStepClick, disabled }) => (
  <div className="flex items-center justify-center w-full px-2 sm:px-0 py-2 gap-x-4 select-none">
    {steps.map((step, idx) => {
      ...
      return (
        <React.Fragment key={step}>
          <div className="flex-1 min-w-0 flex">
            <button
              type="button"
              className={`group flex flex-col items-center justify-center w-full px-4 py-2 border-none outline-none focus:outline-none transition relative rounded-lg ${boxBg} ${clickable ? 'cursor-pointer' : 'cursor-default'} ${idx === current ? (darkMode ? 'z-10' : 'z-10') : ''}`}
              style={{
                border: idx === current
